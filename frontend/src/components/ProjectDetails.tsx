    ...
    <Flex direction={{ base: "column", md: "row" }} gap={8}>
      ...
      <Box flex={1} p={6} bg="white" borderRadius="md" boxShadow="sm">
        ...
        <SimpleGrid columns={[1, 2]} spacing={4} mb={6}>
          ...
          <Box><Text fontWeight="bold">Start Date</Text><Text>{project.startDate ? new Date(project.startDate).toLocaleDateString('en-CA')  : "—"}</Text></Box>
          <Box><Text fontWeight="bold">End Date</Text><Text>{project.endDate ? new Date(project.endDate).toLocaleDateString('en-CA')  : "—"}</Text></Box>
